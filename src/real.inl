...
inline double _double(const real &a) noexcept
{ 
   return a.w; 
}
...
*/
inline real   _real(const double &a) noexcept
{ 
   return real(a); 
}

inline real pred(const real& r) noexcept
{
   real ret;
...
inline real succ(const real& r) noexcept
{
   real ret;
...
inline a_intg expo(const real &r) noexcept
{
   return r_expo(*(a_real*)&r.w);
}

inline real comp(const real &r, a_intg i) noexcept
{
   return real(r_comp(*(a_real*)&r.w,i));
}

inline real mant(const real &r) noexcept
{
   return real(r_mant(*(a_real*)&r.w));
}

inline real operator -(const real &a) noexcept       { return -a.w; }
inline real operator +(const real &a) noexcept       { return a; }
...
inline real abs(const real &a) noexcept
{
  /* if(a.w>=0)
      return a.w;
   return -a.w;*/
...
inline int sign(const real &a) noexcept
{
   if(a.w>0)
      return 1;
   else
...
inline real pow2n(const int n) noexcept // Blomquist 03.10.02.
