...
dotprecision::dotprecision(void) noexcept : akku(new a_btyp[A_LENGTH]), err(0.0), k(0)
{
...
dotprecision::dotprecision(const dotprecision &from) noexcept
                                 : akku(new a_btyp[A_LENGTH]), err(from.err), k(from.k)
{
   memcpy((void *)akku,(void *)from.akku,BUFFERSIZE);
}

dotprecision::dotprecision(const real &r) noexcept
...
dotprecision & dotprecision::operator =(const dotprecision &from) noexcept
{
...
dotprecision & dotprecision::operator =(const real &r) noexcept
{
   real dummy(r);
...
dotprecision operator +(const dotprecision &a,const dotprecision &b) noexcept
{
   dotprecision tmp(a); 
...
dotprecision operator +(const dotprecision &d,const real &r) noexcept
{
   dotprecision erg(d);
...
dotprecision operator +(const real &r,const dotprecision &d) noexcept
{
   dotprecision erg(d);
   d_radd(&(erg.akku),*((a_real *)&r));
   return erg;
...
dotprecision & operator +=(dotprecision &d,const real &r) noexcept
{
   d_radd(&d.akku,*((a_real *)&r));
   return d;
}

dotprecision & operator +=(dotprecision &d,const dotprecision &d2) noexcept
{
...
dotprecision operator -(const dotprecision &a) noexcept
{
   dotprecision tmp(a);
   tmp.negdot();
   return tmp;
...
bool operator ==(const dotprecision &a,const dotprecision &b)  noexcept  // Uebernommen aus dot.cpp "testequal"
{
   int res = true;
...
bool operator <=(const dotprecision &x,const dotprecision &y) noexcept   // Uebernommen aus dot.cpp "testlessequal"
{
   int res = true, cont;
...
bool operator !=(const dotprecision &a,const dotprecision &b) noexcept { return !(a==b); }
bool operator  <(const dotprecision &a,const dotprecision &b) noexcept { return !(b<=a); }
bool operator  >(const dotprecision &a,const dotprecision &b) noexcept { return !(a<=b); }
bool operator >=(const dotprecision &a,const dotprecision &b) noexcept { return (b<=a);  }

bool operator ==(const real &r,const dotprecision &d) noexcept { return dotprecision(r)==d; }
bool operator !=(const real &r,const dotprecision &d) noexcept { return dotprecision(r)!=d; }
bool operator  <(const real &r,const dotprecision &d) noexcept { return dotprecision(r)<d;  }  
bool operator  >(const real &r,const dotprecision &d) noexcept { return dotprecision(r)>d;  }
bool operator <=(const real &r,const dotprecision &d) noexcept { return dotprecision(r)<=d; }
...
bool operator ==(const dotprecision &d,const real &r) noexcept { return d==dotprecision(r); }
bool operator !=(const dotprecision &d,const real &r) noexcept { return d!=dotprecision(r); }
bool operator  <(const dotprecision &d,const real &r) noexcept { return d<dotprecision(r);  }
bool operator  >(const dotprecision &d,const real &r) noexcept { return d>dotprecision(r);  }
bool operator <=(const dotprecision &d,const real &r) noexcept { return d<=dotprecision(r); }
...
bool operator  !(const dotprecision &a) noexcept               { return sign(a)==0; }


void rnd (const dotprecision& d, real& r, rndtype type) noexcept 
{
   if(type==RND_NEXT) {
...
void rnd (const dotprecision& d, real& l, real& u) noexcept 
{
...
void rnd (const dotprecision& d, interval& x) noexcept 
{
    real a,b;
    rnd(d,a,b);
    x = interval(a,b);
...
real rnd (const dotprecision& d, rndtype type) noexcept 
{
    real r;
   if(type==RND_NEXT) {
      *((a_real *)(&r))=d_stan((Dotprecision)d.akku);
...
/*std::string  & operator << (std::string  & s, const dotprecision & d) noexcept 
{
...
std::ostream & operator << (std::ostream & o, const dotprecision & d) noexcept 
{
   std::string buffer;
   buffer << d;
   o << d;
...
std::istream & operator >> (std::istream & i,dotprecision & d) noexcept 
{
...
*/
dotprecision & dotprecision::negdot(void) noexcept 
{
   this->akku[A_SIGN] = 1 - this->akku[A_SIGN];
   return *this;
}

int sign(const dotprecision &a) noexcept 
{
...
dotprecision abs(const dotprecision &a) noexcept 
{
   if(sign(a)>=0)
      return a;
   dotprecision tmp(a);
...
dotprecision & accumulate (dotprecision& dot, const real& a, const real& b) noexcept 
{
