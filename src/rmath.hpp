...
//! Calculates \f$ x^2  \f$
inline real sqr    (const real&) noexcept; // Sqr(x)
...
//! Calculates \f$ \sqrt{1+x^2} \f$
       real sqrt1px2(const real&) noexcept;  // Sqrt(1+x^2); Blomquist 13.12.02
//! Calculates \f$ \sqrt{(x+1)-1} \f$
inline real sqrtm1  (const real&); // Sqrt(x+1)-1; ohne Fehlerabsch�zung!
//! Calculates \f$ \sqrt{(x+1)-1} \f$
       real sqrtp1m1(const real&) noexcept; // Blomquist 05.08.03
...
inline real sin    (const real&) noexcept;        // Sin(x)
//! Calculates \f$ \mbox{sin}(\pi*x)/\pi \f$;
       real sinpix_pi(const real& x);            // sin(pi*x)/pi;
//! Calculates \f$ \cos(x) \f$
inline real cos    (const real&) noexcept;        // Cos(x)
//! Calculates \f$ \tan(x) \f$
inline real tan    (const real&) noexcept;        // Tan(x)
//! Calculates \f$ \cot(x) \f$
inline real cot    (const real&) noexcept;        // Cot(x)
...
inline real exp    (const real&) noexcept;        // Exp(x)
//! Calculates \f$ \exp(x)-1 \f$
inline real expm1  (const real&) noexcept;        // Exp(x)-1
//! Calculates \f$ \exp(-x^2) \f$
       real expmx2 (const real&) noexcept;        // Exp(-x^2)
//! Calculates \f$ \exp(x^2) \f$
...
inline real sinh   (const real&) noexcept;        // Sinh(x)
//! Calculates \f$ \cosh(x) \f$
inline real cosh   (const real&) noexcept;        // Cosh(x)
//! Calculates \f$ \arccos(1+x) \f$
       real acoshp1(const real& x) noexcept;      // acosh(1+x)
//! Calculates \f$ \tanh(x) \f$
inline real tanh   (const real&) noexcept;        // Tanh(x) 
//! Calculates \f$ \coth(x) \f$
inline real coth   (const real&) noexcept;        // Coth(x)
...
//! Calculates \f$ \sqrt{x^2+y^2} \f$
       real sqrtx2y2(const real&, const real&) noexcept; // Sqrt(x^2+y^2)
