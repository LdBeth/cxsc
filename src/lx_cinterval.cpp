...
l_cinterval & l_cinterval::operator = (const lx_cinterval &a) noexcept
...
cinterval & cinterval::operator = (const lx_cinterval &a) noexcept
...
// ------------------------ Input --------------------------------------
/*
std::string & operator >> (std::string &s, lx_cinterval &a) noexcept
...
void operator >> (const std::string &s, lx_cinterval &a) noexcept
{
...
void operator >> (const char *s, lx_cinterval &a) noexcept
{
    std::string r(s);
    r >> a;
}

std::istream & operator >> (std::istream &s, lx_cinterval &a) noexcept
{
    lx_interval Lar, Lai;
    char c;
...
lx_cinterval sqr(const lx_cinterval &z) noexcept
{
...
lx_cinterval sqrt(const lx_cinterval& z) noexcept
{
	lx_cinterval y;
	lx_real
		irez = Inf( Re(z) ),
...
// -------------------------- exp(...) --------------------------------------
	...
	lx_cinterval exp(const lx_cinterval& z) noexcept
	{
  ...
  lx_interval
	...
	lx_cinterval exp2(const lx_cinterval& z) noexcept
	{
		return exp(z*Ln2_lx_interval());
	}

	lx_cinterval exp10(const lx_cinterval& z) noexcept
	{
...
// -------------------------- cos(...) --------------------------------------
	...
	lx_cinterval cos(const lx_cinterval& z) noexcept
	{
		int stagsave = stagprec,
  stagmax = 39;
  if (stagprec > stagmax) 
...
// -------------------------- sin(...) --------------------------------------
	...
	lx_cinterval sin(const lx_cinterval& z) noexcept
	{
		int stagsave = stagprec,
  stagmax = 39;
  if (stagprec > stagmax) 
...
// -------------------------- cosh(...) -------------------------------------
	...
	lx_cinterval cosh(const lx_cinterval& z) noexcept
	{
		int stagsave = stagprec,
  stagmax = 39;
  if (stagprec > stagmax) 
...
// -------------------------- sinh(...) -------------------------------------
	...
	lx_cinterval sinh(const lx_cinterval& z) noexcept
	{
		int stagsave = stagprec,
  stagmax = 39;
  if (stagprec > stagmax) 
...
//
lx_interval Arg(const lx_cinterval& z) noexcept
{
	lx_real 
		srez = Sup( Re(z) ),
		irez = Inf( Re(z) ), 
...
lx_cinterval sqrt(const lx_cinterval& z, int n ) noexcept 
			//
...
//  axis from below;
//
lx_cinterval Ln(const lx_cinterval& z) noexcept
...
//
lx_cinterval log2(const lx_cinterval& z) noexcept
{  // Blomquist, 30.11.2008;
	return Ln(z) / Ln2_lx_interval();
}

lx_cinterval log10(const lx_cinterval& z) noexcept
{  // Blomquist, 30.11.2008;
...
//
lx_cinterval power_fast(const lx_cinterval& z, const real& n) noexcept
{
	if( n == 0 ) return lx_cinterval(lx_interval(1));
...
//
	...
	lx_cinterval power( const lx_cinterval& x, const real& n ) noexcept
	{
		if( !(Is_Integer(n)) )
	//  n is not an integer
...
//  Analytic power function for a real interval exponent, based on Ln.
	...
	lx_cinterval pow( const lx_cinterval& z, const lx_interval& p ) noexcept
	{
		return exp( p*Ln(z) );
	}
...
//  Analytic power function for a complex interval exponent, based on Ln.
	...
	lx_cinterval pow(const lx_cinterval& z, const lx_cinterval& p) noexcept
	{
		return exp( p*Ln(z) );
	}
...
lx_cinterval Tan(const lx_cinterval& z) noexcept 
{
	lx_cinterval y;
	lx_interval
		rez = Re(z),   // rez = z.re(),
...
// Blomquist, 27.09.2007;
  ...
  return y;
	...
	lx_cinterval cot(const lx_cinterval& z) noexcept 
	{
...
//  tanh( z ) = transp( i * tan( transp( i * z ) )
//
lx_cinterval tanh(const lx_cinterval& z) noexcept
{
	int stagsave = stagprec,
  		stagmax = 39; 
  	if (stagprec > stagmax) stagprec = stagmax;
...
lx_cinterval coth(const lx_cinterval& z) noexcept
{ // coth( z ) = i * cot( i * z );
	int stagsave = stagprec,
  		stagmax = 39; 
  	if (stagprec > stagmax) stagprec = stagmax;
...
lx_cinterval asin(const lx_cinterval& z) noexcept 
{
	int stagsave = stagprec,
  		stagmax = 30;
  	if (stagprec>stagmax) stagprec = stagmax;
...
//
lx_cinterval acos(const lx_cinterval& z) noexcept   
{
	lx_interval 
		rez = Re(z),
		imz = Im(z);
...
//-- asinh --------------------------------------------------------------------
//
lx_cinterval asinh( const lx_cinterval& z ) noexcept
//
//  asinh( z ) = i * asin( -i * z )
//
{
...
//-- acosh --------------------------------------------------------------------
//
lx_cinterval acosh( const lx_cinterval& z ) noexcept
//
//  acosh( z ) = i * acos( z ) = +/- i * ( pi / 2 - asin( z ) )
//
{
...
lx_cinterval atan( const lx_cinterval& z ) noexcept
{
	int stagsave = stagprec,
  	stagmax = 30;
  	if (stagprec>stagmax) stagprec = stagmax;
...
lx_cinterval acot( const lx_cinterval& z ) noexcept
{
	int stagsave = stagprec,
  	stagmax = 30;
  	if (stagprec>stagmax) stagprec = stagmax;
...
//-- atanh --------------------------------------------------------------------
//
lx_cinterval atanh( const lx_cinterval& z ) noexcept
//
//  atanh( z ) = - i * atan( i * z )
//
{
...
//-- acoth --------------------------------------------------------------------
//
lx_cinterval acoth( const lx_cinterval& z ) noexcept
//
//  acoth( z ) = i * acot( i * z )
//
{
