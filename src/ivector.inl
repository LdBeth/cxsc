...
namespace cxsc {
	...
	INLINE ivector::ivector () noexcept:dat(NULL),l(1),u(0),size(0)
	{
	}

	INLINE ivector::ivector(const int &i) noexcept:l(1),u(i),size(i)
	{
		dat=new interval[i];
	}

#ifdef OLD_CXSC
	INLINE ivector::ivector(const class index &i) noexcept:l(1),u(i._int()),size(i._int())
	{
	...
	INLINE ivector::ivector(const ivector_slice &rs) noexcept:l(rs.start),u(rs.end),size(rs.end-rs.start+1)
	{
		dat=new interval[size];
	...
	INLINE ivector::ivector(const ivector &v) noexcept:l(v.l),u(v.u),size(v.size)
	{
		dat=new interval[size];
	...
	INLINE ivector::ivector(const interval &r) noexcept:l(1),u(1),size(1)
	{
		dat=new interval[1];
		*dat=r;
	}
	
	INLINE ivector::ivector(const rvector_slice &rs) noexcept:l(rs.start),u(rs.end),size(rs.end-rs.start+1)
	{
		dat=new interval[size];
		for(int i=0, j=l-rs.l;i<size;i++,j++)
			dat[i]=rs.dat[j];
	...
	INLINE ivector::ivector(const rvector &v) noexcept:l(v.l),u(v.u),size(v.size)
	{
		dat=new interval[size];
		for (int i=0;i<size;i++)
			dat[i]=v.dat[i];
	...
	INLINE ivector::ivector(const real &r) noexcept:l(1),u(1),size(1)
	{
		dat=new interval[1];
		*dat=r;
	}
   ...
   INLINE ivector_slice ivector_slice::operator ()(const int &i1,const int &i2)
	...
	\sa ???
	*/
	INLINE ivector _ivector(const interval &r) noexcept { return ivector(r); }
	/*!
	\deprecated use standard contructors for typecasting

	\sa ???
	*/
	INLINE ivector _ivector(const real &r) noexcept { return ivector(r); }
	/*!
	\deprecated use standard contructors for typecasting

	\sa ???
	*/
	INLINE ivector _ivector(const rvector_slice &rs) noexcept { return ivector(rs); }
	/*!
	\deprecated use standard contructors for typecasting

	\sa ???
	*/
	INLINE ivector _ivector(const rvector &rs) noexcept { return ivector(rs); }
	/*!
	\deprecated use standard contructors for typecasting

	\sa ???
	*/
	INLINE ivector _ivector(const rmatrix_subv &rs) noexcept { return ivector(rs); }
	INLINE ivector &ivector::operator =(const ivector &rv) noexcept { return _vvassign<ivector,ivector,interval>(*this,rv); }
	INLINE ivector &ivector::operator =(const interval &r) noexcept { return _vsassign<ivector,interval>(*this,r); }
	INLINE ivector &ivector::operator =(const rvector &rv) noexcept { return _vvassign<ivector,rvector,interval>(*this,rv); }
	INLINE ivector &ivector::operator =(const real &r) noexcept { return _vsassign<ivector,real>(*this,r); }
	INLINE ivector::operator void*() noexcept { return _vvoid(*this); }
	...
	INLINE ivector_slice & ivector_slice::operator =(const interval &r) noexcept { return _vssassign<ivector_slice,interval>(*this,r); }
...
#endif
	{ return _vsvassign(*this,rv); }
	INLINE ivector_slice & ivector_slice::operator =(const real &r) noexcept { return _vssassign(*this,r); }
	INLINE ivector_slice::operator void*() noexcept { return _vsvoid(*this); }
...
//======================== Vector Functions =============================
	...
	INLINE ivector &SetSup(ivector &iv,const real &r) noexcept { return _vssetsup(iv,r); }
	INLINE ivector &SetInf(ivector &iv,const real &r) noexcept { return _vssetinf(iv,r); }
	INLINE ivector &UncheckedSetSup(ivector &iv,const real &r) noexcept { return _vsusetsup(iv,r); }
	INLINE ivector &SetUncheckedInf(ivector &iv,const real &r) noexcept { return _vsusetinf(iv,r); }
	INLINE ivector_slice &SetSup(ivector_slice &iv,const real &r) noexcept { return _vsssetsup(iv,r); }
	INLINE ivector_slice &SetInf(ivector_slice &iv,const real &r) noexcept { return _vsssetinf(iv,r); }
	INLINE ivector_slice &UncheckedSetSup(ivector_slice &iv,const real &r) noexcept { return _vssusetsup(iv,r); }
	INLINE ivector_slice &SetUncheckedInf(ivector_slice &iv,const real &r) noexcept { return _vssusetinf(iv,r); }

	INLINE void Resize(ivector &rv) noexcept { _vresize(rv); } 
	...
	INLINE ivector abs(const ivector &rv) noexcept { return _vabs<ivector,ivector>(rv); }
	INLINE ivector abs(const ivector_slice &sl) noexcept { return _vsabs<ivector_slice,ivector>(sl); }
	INLINE rvector absmin(const ivector &rv) noexcept {
          rvector x(Lb(rv),Ub(rv));
          for(int i=Lb(rv) ; i<=Ub(rv) ; i++)
            x[i] = AbsMin(rv[i]);
          return x;
        }
	INLINE rvector absmin(const ivector_slice &sl) noexcept { 
          rvector x(Lb(sl),Ub(sl));
          for(int i=Lb(sl) ; i<=Ub(sl) ; i++)
            x[i] = AbsMin(sl[i]);
          return x;
        }
	INLINE rvector absmax(const ivector &rv) noexcept {
          rvector x(Lb(rv),Ub(rv));
          for(int i=Lb(rv) ; i<=Ub(rv) ; i++)
            x[i] = AbsMax(rv[i]);
          return x;
        }
	INLINE rvector absmax(const ivector_slice &sl) noexcept { 
          rvector x(Lb(sl),Ub(sl));
          for(int i=Lb(sl) ; i<=Ub(sl) ; i++)
            x[i] = AbsMax(sl[i]);
          return x;
        }
	INLINE rvector diam(const ivector &v) noexcept { return _vdiam<ivector,rvector>(v); }
	INLINE rvector diam(const ivector_slice &v) noexcept { return _vsdiam<ivector_slice,rvector>(v); }
	INLINE rvector mid(const ivector &v) noexcept { return _vmid<ivector,rvector>(v); }
	INLINE rvector mid(const ivector_slice &v) noexcept { return _vsmid<ivector_slice,rvector>(v); }
	INLINE rvector Inf(const ivector &v) noexcept { return _vinf<ivector,rvector>(v); }
	INLINE rvector Inf(const ivector_slice &v) noexcept { return _vsinf<ivector_slice,rvector>(v); }
	INLINE rvector Sup(const ivector &v) noexcept { return _vsup<ivector,rvector>(v); }
	INLINE rvector Sup(const ivector_slice &v) noexcept { return _vssup<ivector_slice,rvector>(v); }
	INLINE bool operator !(const ivector &rv) noexcept { return _vnot(rv); }
	INLINE bool operator !(const ivector_slice &sl) noexcept { return _vsnot(sl); }

//======================= Vector / Scalar ===============================

//----------------------------- Interval ---------------------------

	INLINE ivector operator *(const ivector &rv, const interval &s) noexcept { return _vsmult<ivector,interval,ivector>(rv,s); }
	INLINE ivector operator *(const ivector_slice &sl, const interval &s) noexcept { return _vssmult<ivector_slice,interval,ivector>(sl,s); }
	INLINE ivector operator *(const interval &s, const ivector &rv) noexcept { return _vsmult<ivector,interval,ivector>(rv,s); }
	INLINE ivector operator *(const interval &s, const ivector_slice &sl) noexcept { return _vssmult<ivector_slice,interval,ivector>(sl,s); }
	INLINE ivector &operator *=(ivector &rv,const interval &r) noexcept { return _vsmultassign(rv,r); }
	INLINE ivector_slice &ivector_slice::operator *=(const interval &r) noexcept { return _vssmultassign(*this,r); }

	INLINE ivector operator /(const ivector &rv, const interval &s) noexcept { return _vsdiv<ivector,interval,ivector>(rv,s); }
	INLINE ivector operator /(const ivector_slice &sl, const interval &s) noexcept { return _vssdiv<ivector_slice,interval,ivector>(sl,s); }
	INLINE ivector &operator /=(ivector &rv,const interval &r) noexcept { return _vsdivassign(rv,r); }
	INLINE ivector_slice &ivector_slice::operator /=(const interval &r) noexcept { return _vssdivassign(*this,r); }

//---------------------------- Real --------------------------------------

	INLINE ivector operator *(const ivector &rv, const real &s) noexcept { return _vsmult<ivector,real,ivector>(rv,s); }
	INLINE ivector operator *(const ivector_slice &sl, const real &s) noexcept { return _vssmult<ivector_slice,real,ivector>(sl,s); }
	INLINE ivector operator *(const real &s, const ivector &rv) noexcept { return _vsmult<ivector,real,ivector>(rv,s); }
	INLINE ivector operator *(const real &s, const ivector_slice &sl) noexcept { return _vssmult<ivector_slice,real,ivector>(sl,s); }
	INLINE ivector &operator *=(ivector &rv,const real &r) noexcept { return _vsmultassign(rv,r); }
	INLINE ivector_slice &ivector_slice::operator *=(const real &r) noexcept { return _vssmultassign(*this,r); }

	INLINE ivector operator /(const ivector &rv, const real &s) noexcept { return _vsdiv<ivector,real,ivector>(rv,s); }
	INLINE ivector operator /(const ivector_slice &sl, const real &s) noexcept { return _vssdiv<ivector_slice,real,ivector>(sl,s); }
	INLINE ivector &operator /=(ivector &rv,const real &r) noexcept { return _vsdivassign(rv,r); }
	INLINE ivector_slice &ivector_slice::operator /=(const real &r) noexcept { return _vssdivassign(*this,r); }

	INLINE ivector operator *(const rvector &rv, const interval &s) noexcept { return _vsmult<rvector,interval,ivector>(rv,s); }
	INLINE ivector operator *(const rvector_slice &sl, const interval &s) noexcept { return _vssmult<rvector_slice,interval,ivector>(sl,s); }
	INLINE ivector operator *(const interval &s, const rvector &rv) noexcept { return _vsmult<rvector,interval,ivector>(rv,s); }
	INLINE ivector operator *(const interval &s, const rvector_slice &sl) noexcept { return _vssmult<rvector_slice,interval,ivector>(sl,s); }

	INLINE ivector operator /(const rvector &rv, const interval &s) noexcept { return _vsdiv<rvector,interval,ivector>(rv,s); }
	INLINE ivector operator /(const rvector_slice &sl, const interval &s) noexcept { return _vssdiv<rvector_slice,interval,ivector>(sl,s); }

//======================= Vector / Vector ===============================


	INLINE std::ostream &operator <<(std::ostream &s, const ivector &rv) noexcept { return _vout(s,rv); }
	INLINE std::ostream &operator <<(std::ostream &o, const ivector_slice &sl) noexcept { return _vsout(o,sl); }
	INLINE std::istream &operator >>(std::istream &s, ivector &rv) noexcept { return _vin(s,rv); }
	INLINE std::istream &operator >>(std::istream &s, ivector_slice &rv) noexcept { return _vsin(s,rv); }
	
//----------------------- Interval / Interval ---------------------------
	INLINE ivector & ivector::operator =(const ivector_slice &sl) noexcept { return _vvsassign<ivector,ivector_slice,interval>(*this,sl); }
	...
	INLINE const ivector &operator +(const ivector &rv) noexcept { return rv; }
	INLINE ivector operator +(const ivector_slice &sl) noexcept { return sl; }
	...
	INLINE ivector operator -(const ivector &rv) noexcept { return _vminus(rv); }
	INLINE ivector operator -(const ivector_slice &sl) noexcept { return _vsminus<ivector_slice,ivector>(sl); }
	INLINE ivector operator -(const ivector &rv1, const ivector &rv2)
#if(CXSC_INDEX_CHECK)
	noexcept(false)
#else
	...
	INLINE bool operator ==(const ivector &rv1, const ivector &rv2) noexcept { return _vveq(rv1,rv2); }
	INLINE bool operator ==(const ivector_slice &sl1, const ivector_slice &sl2) noexcept { return _vsvseq(sl1,sl2); }
	INLINE bool operator ==(const ivector_slice &sl, const ivector &rv) noexcept { return _vsveq(sl,rv); }
	INLINE bool operator ==(const ivector &rv, const ivector_slice &sl) noexcept { return _vsveq(sl,rv); }
	INLINE bool operator !=(const ivector &rv1, const ivector &rv2) noexcept { return _vvneq(rv1,rv2); }
	INLINE bool operator !=(const ivector_slice &sl1, const ivector_slice &sl2) noexcept { return _vsvsneq(sl1,sl2); }
	INLINE bool operator !=(const ivector_slice &sl, const ivector &rv) noexcept { return _vsvneq(sl,rv); }
	INLINE bool operator !=(const ivector &rv, const ivector_slice &sl) noexcept { return _vsvneq(sl,rv); }
	INLINE bool operator <(const ivector &rv1, const ivector &rv2) noexcept { return _vvless(rv1,rv2); }
	INLINE bool operator <(const ivector_slice &sl1, const ivector_slice &sl2) noexcept { return _vsvsless(sl1,sl2); }
	INLINE bool operator < (const ivector_slice &sl, const ivector &rv) noexcept { return _vsvless(sl,rv); }
	INLINE bool operator < (const ivector &rv, const ivector_slice &sl) noexcept { return _vvsless(rv,sl); }
	INLINE bool operator <=(const ivector &rv1, const ivector &rv2) noexcept { return _vvleq(rv1,rv2); }
	INLINE bool operator <=(const ivector_slice &sl1, const ivector_slice &sl2) noexcept { return _vsvsleq(sl1,sl2); }
	INLINE bool operator <=(const ivector_slice &sl, const ivector &rv) noexcept { return _vsvleq(sl,rv); }
	INLINE bool operator <=(const ivector &rv, const ivector_slice &sl) noexcept { return _vvsleq(rv,sl); }
	INLINE bool operator >(const ivector &rv1, const ivector &rv2) noexcept { return _vvless(rv2,rv1); }
	INLINE bool operator >(const ivector_slice &sl1, const ivector_slice &sl2) noexcept { return _vsvsless(sl2,sl1); }
	INLINE bool operator >(const ivector_slice &sl, const ivector &rv) noexcept { return _vvsless(rv,sl); }
	INLINE bool operator >(const ivector &rv, const ivector_slice &sl) noexcept { return _vsvless(sl,rv); }
	INLINE bool operator >=(const ivector &rv1, const ivector &rv2) noexcept { return _vvleq(rv2,rv1); }
	INLINE bool operator >=(const ivector_slice &sl1, const ivector_slice &sl2) noexcept { return _vsvsleq(sl2,sl1); }
	INLINE bool operator >=(const ivector_slice &sl, const ivector &rv) noexcept { return _vvsleq(rv,sl); }
	INLINE bool operator >=(const ivector &rv, const ivector_slice &sl) noexcept { return _vsvleq(sl,rv); }

//-------------------------------- Interval / Real --------------------------------

	INLINE ivector & ivector::operator =(const rvector_slice &sl) noexcept { return _vvsassign<ivector,rvector_slice,interval>(*this,sl); }
